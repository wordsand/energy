...
function getSubDevices($conn) {
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<tbody>";
        while ($row = $result->fetch_assoc()) {
            echo "<tr><td>" .$row['ID'] . "</td><td>" . $row['名称'] . "</td><td>" . $row['编号'] . "</td><td>" . $row['类别'] . "</td><td>" . $row['规格型号'] . "</td><td>" . $row['有效期'] . "</td><td>" . $row['数量'] . "</td><td>" . $row['计量单位'] . "</td>";

            echo "<td><button type='button' class='btn btn-sm btn-warning change-subdevice' data-id='" . $row['ID'] . "'>变更</button> ";


            echo "<button type='button' class='btn btn-sm btn-info transfer-subdevice' data-id='" . $row['ID'] . "'>调拨</button></td>";
...
function updateSubDevice($conn) {
    ...
    if ($ID !== '' && $名称 !== '' && $编号 !== '') {
        $sql = "UPDATE Sub SET 名称='$名称', 编号='$编号' WHERE ID='$ID'";
        // 更新其他字段，如类别、型号规格、有效期等
        if ($conn->query($sql) === TRUE) {
            echo 'success';
        } else {
            echo "Error: " . $sql . "<br>" . $conn->error;
...
function getTransferSubDeviceForm($conn, $subId) {
    ...
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        echo "<h3>调拨附件</h3>";
        echo "<form id='transfer-sub-device-form'>";
...
function transferSubDevice($conn) {
    $ID = isset($_POST['ID']) ? $_POST['ID'] : '';
    $target_main_id = isset($_POST['target_main_id']) ? $_POST['target_main_id'] : '';

    if ($ID !== '' && $target_main_id !== '') {
        $sql = "UPDATE Sub SET mainId='$target_main_id' WHERE ID='$ID'";
        if ($conn->query($sql) === TRUE) {
            echo 'success';
        } else {
            echo "Error: " . $sql . "<br>" . $conn->error;
